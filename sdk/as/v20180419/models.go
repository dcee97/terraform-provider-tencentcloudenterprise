...
type RemoveSelectedInstanceResponse struct {
	*tchttp.BaseResponse
...
func (r *RemoveSelectedInstanceResponse) ToJsonString() string {
	b, _ := json.Marshal(r)
	return string(b)
}

func (r *RemoveSelectedInstanceResponse) FromJsonString(s string) error {
	return json.Unmarshal([]byte(s), &r)
}
...
type CreateLaunchConfigurationRequest struct {
	*tchttp.BaseRequest
	...
	// 实例所属项目ID。该参数可以通过调用 [DescribeProjects](/tcloud/api/TCenter_CAT/TCENTER_SUBCAT/APIs/组织与项目（tpo）/版本（2020-09-20）/Project相关接口/DescribeProjects) 的返回值中的`projectId`字段来获取。不填为默认项目。

	ProjectId *uint64 `json:"ProjectId,omitempty" name:"ProjectId"`
	// 实例机型。不同实例机型指定了不同的资源规格，具体取值可通过调用接口 [DescribeInstanceTypeConfigs](/tcloud/api/Compute/CVM/APIs/云服务器（cvm）/版本（2017-03-12）/实例相关接口/DescribeInstanceTypeConfigs) 来获得最新的规格表或参见[实例类型](/tcloud/Compute/CVM/292128/484318/specification)描述。
	// `InstanceType`和`InstanceTypes`参数互斥，二者必填一个且只能填写一个。

	InstanceType *string `json:"InstanceType,omitempty" name:"InstanceType"`
	// 指定有效的[镜像]ID，格式形如`img-8toqc6s3`。镜像类型分为三种：<br/><li>公共镜像</li><li>自定义镜像</li><li>共享镜像</li><br/>可通过以下方式获取可用的镜像ID：<br/><li>`公共镜像`、`自定义镜像`、`共享镜像`的镜像ID可通过登录[控制台]查询。</li><li>通过调用接口 [DescribeImages]，取返回信息中的`ImageId`字段。</li>
	...
	// 实例所属安全组。该参数可以通过调用 [DescribeSecurityGroups](/tcloud/api/NetWork/VPC/APIs/私有网络（vpc）/版本（2017-03-12）/安全组相关接口/DescribeSecurityGroups) 的返回值中的`SecurityGroupId`字段来获取。若不指定该参数，则默认不绑定安全组。
...
type SystemDisk struct {
	...
	// 系统盘类型。取值范围：<br><li>LOCAL_BASIC：本地硬盘<br><li>LOCAL_SSD：本地SSD硬盘<br><li>CLOUD_BASIC：普通云硬盘<br><li>CLOUD_PREMIUM：高性能云硬盘<br><li>CLOUD_SSD：SSD云硬盘<br><br>默认取值：LOCAL_BASIC。
...
type RemoveSelectedInstanceRequest struct {
	*tchttp.BaseRequest
	...
	DetachInstanceIds []*string `json:"DetachInstanceIds,omitempty" name:"DetachInstanceIds"`
	// CVM删除实例ID列表

	RemoveInstanceIds []*string `json:"RemoveInstanceIds,omitempty" name:"RemoveInstanceIds"`
}

func (r *RemoveSelectedInstanceRequest) ToJsonString() string {
	b, _ := json.Marshal(r)
	return string(b)
}

func (r *RemoveSelectedInstanceRequest) FromJsonString(s string) error {
	return json.Unmarshal([]byte(s), &r)
}
...
type DataDisk struct {
	...
	// 数据盘类型。取值范围：<br><li>LOCAL_BASIC：本地硬盘<br><li>LOCAL_SSD：本地SSD硬盘<br><li>CLOUD_BASIC：普通云硬盘<br><li>CLOUD_PREMIUM：高性能云硬盘<br><li>CLOUD_SSD：SSD云硬盘<br><br>默认取值：LOCAL_BASIC。
...
type CreatePaiInstanceResponse struct {
	*tchttp.BaseResponse
	...
	Response *struct {
		// 当通过本接口来创建实例时会返回该参数，表示一个或多个实例`ID`。返回实例`ID`列表并不代表实例创建成功，可根据 [DescribeInstances](/tcloud/api/Compute/CVM/APIs/云服务器（cvm）/版本（2017-03-12）/实例相关接口/DescribeInstances) 接口查询返回的InstancesSet中对应实例的`ID`的状态来判断创建是否完成；如果实例状态由“准备中”变为“正在运行”，则为创建成功。
...
type UpgradeLaunchConfigurationRequest struct {
	*tchttp.BaseRequest
	...
	EnhancedService *EnhancedService `json:"EnhancedService,omitempty" name:"EnhancedService"`
	// 指定有效的[镜像](/tcloud/Compute/CVM/292128/835305/mirr_overview)ID，格式形如`img-8toqc6s3`。镜像类型分为四种：<br/><li>公共镜像</li><li>自定义镜像</li><li>共享镜像</li><li>服务市场镜像</li><br/>可通过以下方式获取可用的镜像ID：<br/><li>`公共镜像`、`自定义镜像`、`共享镜像`的镜像ID可通过登录[控制台](//console.{{conf.main_domain}}/cvm/image/list?imageType=PUBLIC_IMAGE&pageIndex=1&pageSize=20)查询。</li><li>通过调用接口 [DescribeImages](/tcloud/api/Compute/CVM/APIs/云服务器（cvm）/版本（2017-03-12）/镜像相关接口/DescribeImages) ，取返回信息中的`ImageId`字段。</li>
