...
func NewRemoveSelectedInstanceRequest() (request *RemoveSelectedInstanceRequest) {
	request = &RemoveSelectedInstanceRequest{
		BaseRequest: &tchttp.BaseRequest{},
	}
	request.Init().WithApiInfo("as", APIVersion, "RemoveSelectedInstance")
	return
}

func NewRemoveSelectedInstanceResponse() (response *RemoveSelectedInstanceResponse) {
	response = &RemoveSelectedInstanceResponse{
		BaseResponse: &tchttp.BaseResponse{},
	}
	return
}

// 本接口（RemoveSelectedInstance）用于从伸缩组删除和移出 CVM 实例。
func (c *Client) RemoveSelectedInstance(request *RemoveSelectedInstanceRequest) (response *RemoveSelectedInstanceResponse, err error) {
	if request == nil {
		request = NewRemoveSelectedInstanceRequest()
	}
	response = NewRemoveSelectedInstanceResponse()
	err = c.Send(request, response)
	return
}
