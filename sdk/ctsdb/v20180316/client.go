...
func NewDescribeShrinkableDBInstanceNodeRequest() (request *DescribeShrinkableDBInstanceNodeRequest) {
	request = &DescribeShrinkableDBInstanceNodeRequest{
		BaseRequest: &tchttp.BaseRequest{},
	}
	request.Init().WithApiInfo("ctsdb", APIVersion, "DescribeShrinkableDBInstanceNode")
	return
}

func NewDescribeShrinkableDBInstanceNodeResponse() (response *DescribeShrinkableDBInstanceNodeResponse) {
	response = &DescribeShrinkableDBInstanceNodeResponse{
		BaseResponse: &tchttp.BaseResponse{},
	}
	return
}

// 横向缩容时，查询实例可缩容节点信息，如可缩容节点数
func (c *Client) DescribeShrinkableDBInstanceNode(request *DescribeShrinkableDBInstanceNodeRequest) (response *DescribeShrinkableDBInstanceNodeResponse, err error) {
	if request == nil {
		request = NewDescribeShrinkableDBInstanceNodeRequest()
	}
	response = NewDescribeShrinkableDBInstanceNodeResponse()
	err = c.Send(request, response)
	return
}
