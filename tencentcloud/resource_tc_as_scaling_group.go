...
func init() {
	registerResourceDescriptionProvider("cloud_as_scaling_group", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"termination_policies":            "销毁策略",
			"retry_policy":                    "重试策略",
			"scaling_mode":                    "伸缩模式",
			"replace_monitor_unhealthy":       "替换监控不健康实例",
			"replace_load_balancer_unhealthy": "替换负载均衡不健康实例",
			"tags":                            "标签",
			"status":                          "伸缩组状态",
			"instance_count":                  "实例数量",
			"create_time":                     "创建时间",
			"multi_zone_subnet_policy":        "多可用区或子网策略",
			"load_balancer_ids":               "传统负载均衡ID列表",
			"zones":                           "可用区列表",
			"forward_balancer_ids":            "应用负载均衡ID列表",
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"subnet_ids": {
				...
				Description: "ID list of subnet, for VPC it is required.",
			},
			"zones": {
				Type:        schema.TypeList,
				Optional:    true,
