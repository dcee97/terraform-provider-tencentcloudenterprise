...
func init() {
	registerResourceDescriptionProvider("cloud_cos_bucket", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"lifecycle_rules":           "生命周期规则，请至少填写一项非必选参数：non_current_transition、non_current_expiration、transition和expiration",
			"filter_prefix":             "前缀匹配策略",
			"filter_tags":               "标签匹配策略，用于根据对象标签过滤",
			"transition":                "当前版本文件沉降配置",
			...
			"non_current_transition": "非当前版本文件沉降配置",
...
func resourceTencentCloudCosBucketCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err := WaitCosBucketCreated(ctx, cosService, bucket); err != nil {
		return err
	}
	d.SetId(bucket)
...
func resourceTencentCloudCosBucketRead(d *schema.ResourceData, meta interface{}) error {
	...
	cosService := CosService{client: client, useCspClient: false}
	...
	cosBucketUrl := fmt.Sprintf("%s.cos.%s.%s", bucket, client.Region, client.CosDomain)
	...
	//	endPointUrl := fmt.Sprintf("%s.cos-website.cos.%s.%s", d.Id(), client.Region, client.CosDomain)
...
func resourceTencentCloudCosBucketUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	client := meta.(*TencentCloudClient).apiV3Conn.UseCosS3Client(false)
...
func resourceTencentCloudCosBucketDelete(d *schema.ResourceData, meta interface{}) error {
	...
	cosService := CosService{
		client: meta.(*TencentCloudClient).apiV3Conn, useCspClient: false,
	}
...
func resourceTencentCloudCosBucketLifecycleUpdate(ctx context.Context, client *s3.S3, d *schema.ResourceData) error {
	...
	if len(lifecycleRules) == 0 {
		...
		for i, lifecycleRule := range lifecycleRules {
			...
			filterTags := r["filter_tags"].(map[string]interface{})

			if prefix != "" && len(filterTags) > 0 {
				// Both prefix and tags specified, use And operator
				andOperator := &s3.LifecycleRuleAndOperator{
					Prefix: &prefix,
				}

				// Add tags to And operator
				if len(filterTags) > 0 {
					tags := make([]*s3.Tag, 0, len(filterTags))
				...
				// No filter specified, use empty filter
				rule.Filter = &s3.LifecycleRuleFilter{}
			}
