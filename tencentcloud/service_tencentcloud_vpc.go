...
func (me *VpcService) CreateSubnet(ctx context.Context, vpcId, name, cidr, zone string, tags map[string]string,
	vpc_type uint64) (subnetId string, errRet error) {
	logId := getLogId(ctx)
	request := vpc.NewCreateSubnetsRequest()
	defer func() {
		if errRet != nil {
			log.Printf("[CRITAL]%s api[%s] fail, request body [%s], reason[%s]\n",
				logId, request.GetAction(), request.ToJsonString(), errRet.Error())
	...
	request.VpcId = &vpcId
	request.Subnets = []*vpc.SubnetInput{
		{
			SubnetName: &name,
			CidrBlock:  &cidr,
			Zone:       &zone,
	...
	var response *vpc.CreateSubnetsResponse
	if err := resource.Retry(readRetryTimeout, func() *resource.RetryError {
		ratelimit.Check(request.GetAction())
		result, err := me.client.UseVpcClient().CreateSubnets(request)
		if err != nil {
			return retryError(err)
		}
		response = result
	...
	log.Printf("INFO %s create subnet success, subnetId[%s]", logId, subnetId)
	return
}
...
func (me *VpcService) DeleteRoutes(ctx context.Context, routeTableId string, entryId uint64) (errRet error) {
	...
	request.RouteTableId = &routeTableId
	var route vpc.DellRoute
	route.RouteId = &entryId
	request.Routes = []*vpc.DellRoute{&route}
	ratelimit.Check(request.GetAction())
...
func (me *VpcService) DescribeSecurityGroup(ctx context.Context, id string) (sg *vpc.SecurityGroup, err error) {
	...
	if err := resource.Retry(readRetryTimeout, func() *resource.RetryError {
		...
		if err != nil {
			if sdkError, ok := err.(*sdkErrors.CloudSDKError); ok {
...
/*
func (me *VpcService) CreateVpnGatewayRoute(ctx context.Context, vpnGatewayId string,
	vpnGwRoutes []*vpc.VpnGatewayRoute) (errRet error, routes []*vpc.VpnGatewayRoute) {
	logId := getLogId(ctx)
...
func (me *VpcService) DescribeDirectConnectGateways(ctx context.Context, id string) (
	infos []*vpc.DirectConnectGateway, errRet error) {
	...
	request.DirectConnectGatewayIds = []*string{&id}
	...
	infos = response.Response.DirectConnectGatewaySet
...
// CreateDirectConnectGateway 创建专线网关
func (me *VpcService) CreateDirectConnectGateway(ctx context.Context, name,
	gatewayType, networkInstanceId, networkType string, bandwith int64) (directConnectGatewayId string,
	errRet error) {
	log.Printf("[Info] CreateDirectConnectGateway start params are: [%s] [%s] [%s] [%s] [%d]\n",
		name, gatewayType, networkInstanceId, networkType, bandwith)
	logId := getLogId(ctx)
	...
	if response.Response == nil || response.Response.DirectConnectGateway == nil {
		errRet = fmt.Errorf("response is nil")
		return
	}
	directConnectGatewayId = *response.Response.DirectConnectGateway.DirectConnectGatewayId
	return
}

// DescribeDirectConnectGatewayById 查询专线网关
func (me *VpcService) DescribeDirectConnectGatewayById(ctx context.Context, directConnectGatewayId string) (
	directConnectGateway *vpc.DirectConnectGateway, errRet error) {
	logId := getLogId(ctx)
	request := vpc.NewDescribeDirectConnectGatewaysRequest()
	...
	if len(response.Response.DirectConnectGatewaySet) > 0 {
		directConnectGateway = response.Response.DirectConnectGatewaySet[0]
	}
	return
}

// DeleteDirectConnectGateway 删除专线网关
func (me *VpcService) DeleteDirectConnectGateway(ctx context.Context, directConnectGatewayId string) (
	errRet error) {
	logId := getLogId(ctx)
	...
	ratelimit.Check(request.GetAction())
	_, err := me.client.UseVpcClient().DeleteDirectConnectGateway(request)
	if err != nil {
		errRet = err
	}
	return
...
// ModifyDirectConnectGatewayAttribute 修改专线网关属性
func (me *VpcService) ModifyDirectConnectGatewayAttribute(ctx context.Context, directConnectGatewayId, name string,
	bandwith int64) (
	errRet error) {
	logId := getLogId(ctx)
	...
	ratelimit.Check(request.GetAction())
	_, err := me.client.UseVpcClient().ModifyDirectConnectGatewayAttribute(request)
	if err != nil {
		errRet = err
	}
	return
...
func (me *VpcService) DeleteVpcPeerConnectExManagerById(ctx context.Context, peeringConnectionId string) (errRet error) {
	logId := getLogId(ctx)

	request := vpc.NewDeleteVpcPeeringConnectionExRequest()
	request.PeeringConnectionId = &peeringConnectionId
	...
	response, err := me.client.UseVpcClient().DeleteVpcPeeringConnectionEx(request)
	if err != nil {
		errRet = err
		return
	}
