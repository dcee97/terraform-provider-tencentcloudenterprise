...
func resourceTencentCloudCosBucketObjectCreate(d *schema.ResourceData, meta interface{}) error {
	...
	response, err := meta.(*TencentCloudClient).apiV3Conn.UseCosS3Client(false).PutObject(request)
	if err != nil {
	...
	if err = WaitCosBucketObjectCreated(contextNil, &cosService, bucket, key); err != nil {
		return err
	}
...
// WaitCosBucketObjectCreated wait for the object to be created in the bucket
func WaitCosBucketObjectCreated(ctx context.Context, service *CosService, bucket, key string) error {
	logId := getLogId(ctx)
