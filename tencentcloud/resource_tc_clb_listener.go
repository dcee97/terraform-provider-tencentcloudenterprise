...
func init() {
	registerResourceDescriptionProvider("cloud_clb_listener", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"port":                       "监听器端口",
			"protocol":                   "监听器协议",
			"health_check_switch":        "是否开启健康检查",
			"health_check_time_out":      "健康检查响应超时时间",
			"health_check_interval_time": "健康检查间隔时间",
			"health_check_health_num":    "健康阈值",
			"health_check_unhealth_num":  "不健康阈值",
			"health_check_type":          "健康检查协议",
			"health_check_port":          "健康检查端口",
			"health_check_http_version":  "健康检查域名",
			"health_check_http_code":     "健康检查返回码",
			"health_check_http_path":     "健康检查路径",
			"health_check_http_domain":   "健康检查域名",
			"health_check_http_method":   "健康检查方法",
			"health_check_context_type":  "健康检查协议",
			"health_check_send_context":  "健康检查发送内容",
			"health_check_recv_context":  "健康检查返回内容",
			"certificate_ssl_mode":       "证书类型",
			"certificate_id":             "服务端证书ID",
			"certificate_ca_id":          "客户端证书ID",
			"session_expire_time":        "会话保持时间",
			"scheduler":                  "调度算法",
			"sni_switch":                 "是否开启SNI",
			"target_type":                "后端目标类型",
			"listener_id":                "监听器ID",
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"protocol": {
				...
				Description:  "Type of protocol within the listener. Valid values: `TCP`, `UDP`, `HTTP`, `HTTPS`.",
			},
			"health_check_switch": {
			...
			"health_check_time_out": {
				...
				Description:  "Response timeout of health check. Valid value ranges: [2~60] sec. Default is 2 sec. Response timeout needs to be less than check interval. NOTES: Only supports listeners of `TCP`,`UDP` protocol.",
			},
			...
			"certificate_ssl_mode": {
				...
				Description:  "Type of certificate. Valid values: `UNIDIRECTIONAL`, `MUTUAL`. NOTES: Only supports listeners of `HTTPS` protocol and must be set when it is available.",
			},
			"certificate_id": {
				Type:     schema.TypeString,
				Optional: true,
				Description: "ID of the server certificate." +
					" NOTES: Only supports listeners of `HTTPS` protocol and must be set when it is available.",
			},
			"certificate_ca_id": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "ID of the client certificate. NOTES: Only supports listeners of `HTTPS` protocol and must be set when the ssl mode is `MUTUAL`.",
			},
			"session_expire_time": {
				Type:     schema.TypeInt,
				Optional: true,
...
func resourceTencentCloudClbListenerCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("session_expire_time"); ok {
		...
		vv := int64(v.(int))
		if !(vv == 0 || vv >= 30 && vv <= 3600) {
			return fmt.Errorf("[CHECK][CLB listener][Create] check: " +
				"session_expire_time must be in range [30, 3600], or 0 means close session sticky")
		}
...
func resourceTencentCloudClbListenerUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("scheduler") {
		...
		if !(protocol == CLB_LISTENER_PROTOCOL_TCP || protocol == CLB_LISTENER_PROTOCOL_UDP || protocol == CLB_LISTENER_PROTOCOL_TCPSSL) {
			return fmt.Errorf("[CHECK][CLB listener %s][Update] check: Scheduler can only be set with listener protocol TCP/UDP or rule of listener HTTP/HTTPS", listenerId)
		}
