...
func init() {
	registerResourceDescriptionProvider("cloud_tke_kubernetes_serverless_node_pool", CNDescription{
		...
		AttributesCN: map[string]string{
			"cluster_id":         "集群ID",
			"name":               "节点池名称",
			"serverless_nodes":   "节点列表",
			"display_name":       "serverless节点名称",
			"subnet_id":          "serverless节点子网ID",
...
func resourceTkeServerLessNodePool() *schema.Resource {
	return &schema.Resource{
		...
		Description: "Provides a resource to create and manage TKE serverless node pool",
