...
func init() {
	registerDataDescriptionProvider("cloud_clb_instance_by_cert_id", CNDescription{
		...
		AttributesCN: map[string]string{
			"cert_ids":                     "服务器或客户端证书ID",
			"cert_set":                     "证书ID和与其关联的CLB实例列表",
			"cert_id":                      "证书ID",
			"load_balancers":               "与证书关联的CLB实例的列表注意：此字段可能返回null，表示无法获得有效值",
			"load_balancer_id":             "CLB实例ID",
			"load_balancer_name":           "CLB实例名称",
			"load_balancer_type":           "CLB实例网络类型：OPEN：公网；内部：专用网络",
			"forward":                      "CLB类型标识符取值范围：1（CLB）；0（经典CLB）",
			"domain":                       "CLB实例的域名它只适用于公共的经典CLB此参数将很快停用请改用LoadBalancerDomain注意：此字段可能返回null，表示无法获得有效值",
			"load_balancer_vips":           "CLB实例的VIP列表注意：此字段可能返回null，表示无法获得有效值",
			"status":                       "CLB实例状态，包括：0:正在创建；1：正在运行注意：此字段可能返回null，表示无法获得有效值",
			"create_time":                  "CLB实例创建时间注意：此字段可能返回null，表示无法获得有效值",
			"status_time":                  "CLB实例的上次状态更改时间注意：此字段可能返回null，表示无法获得有效值",
			"project_id":                   "CLB实例所属项目的ID0：默认项目",
			"vpc_id":                       "VPC ID注意：此字段可能返回null，表示无法获取有效值",
			"open_bgp":                     "保护CLB标识符数值范围：1（保护性），0（非保护性）注意：此字段可能返回null，表示无法获得有效值",
			"snat":                         "SNAT适用于2016年12月之前创建的所有专用网络经典CLB注意：此字段可能返回null，表示无法获得有效值",
			"isolation":                    "0：未隔离；1：隔离注意：此字段可能返回null，表示无法获得有效值",
			"log":                          "日志信息只有具有HTTP或HTTPS侦听器的公共网络CLB才能生成日志注意：此字段可能返回null，表示无法获得有效值",
			"subnet_id":                    "CLB实例所在的子网（仅对专有网络VPC CLB有意义）注意：此字段可能返回null，表示无法获得有效值",
			"tags":                         "CLB实例标记信息注意：此字段可能返回null，表示无法获得有效值",
			"tag_key":                      "标记键",
			"tag_标记值":                      "标记值",
			"secure_groups":                "CLB实例的安全组注意：此字段可能返回null，表示无法获得有效值",
			"target_region_info":           "绑定到CLB实例的后端服务器的基本信息注意：此字段可能返回null，表示无法获得有效值",
			"region":                       "目标区域，如广州亚太区",
			"anycast_zone":                 "Anycast CLB出版区域对于非anycast CLB，此字段返回一个空字符串注意：此字段可能返回null，表示无法获得有效值",
			"address_i_p_version":          "IP版本有效值：ipv4、ipv6注意：此字段可能返回null，表示无法获得有效值",
			"numerical_vpc_id":             "VPC ID为数字形式注意：此字段可能返回null，表示无法获得有效值",
			"vip_isp":                      "CLB IP地址所属的ISP注意：此字段可能返回null，表示无法获得有效值",
			"master_zone":                  "主AZ注意：此字段可能返回null，表示无法获得有效值",
			"zone_id":                      "",
			"zone":                         "数字形式的唯一AZ ID，例如100001注意：此字段可能返回null，表示无法获得有效值",
			"zone_name":                    "AZ名称，如广州1区注意：此字段可能返回null，表示无法获得有效值",
			"zone_region":                  "AZ地区，例如广州注意：此字段可能返回null，表示无法获得有效值",
			"local_zone":                   "AZ是否为LocalZone，例如false注意：此字段可能返回null，表示无法获得有效值",
			"edge_zone":                    "AZ是否为边缘区域值：true，false注意：此字段可能返回null，表示无法获得有效值",
			"backup_zone_set":              "备份区域",
			"isolated_time":                "CLB实例隔离时间注意：此字段可能返回null，表示无法获得有效值",
			"expire_time":                  "CLB实例过期时间，仅对预付费实例生效注意：此字段可能返回null，表示无法获得有效值",
			"charge_type":                  "CLB实例的计费模式有效值：PREPAID（每月订阅）、POSTPAID_BY_HOUR（现收现付）注意：此字段可能返回null，表示无法获得有效值",
			...
			"bandwidthpkg_sub_type":        "带宽包类型，例如SINGLIESP注意：此字段可能返回null，表示无法获得有效值",
			"prepaid_attributes":           "CLB实例的预付费计费属性注意：此字段可能返回null，表示无法获得有效值",
			"renew_flag":                   "续订类型AUTO_RENEW：自动续费；MANUAL_NEW：手动续订注意：此字段可能返回null，表示无法获得有效值",
			"period":                       "周期，表示月数（保留字段）注意：此字段可能返回null，表示无法获得有效值",
			"log_set_id":                   "CLB日志服务（CLS）的日志集ID注意：此字段可能返回null，表示无法获得有效值",
			"log_topic_id":                 "CLB日志服务（CLS）的日志主题ID注意：此字段可能返回null，表示无法获得有效值",
			"address_i_pv6":                "CLB实例的IPv6地址注意：此字段可能返回null，表示无法获得有效值",
			"extra_info":                   "保留字段，通常可以忽略注意：此字段可能返回null，表示无法获得有效值",
			"zhi_tong":                     "是否启用VIP直连注意：此字段可能返回null，表示无法获得有效值",
			"tgw_group_name":               "TgwGroup名称注意：此字段可能返回null，表示无法获得有效值",
			"is_ddos":                      "是否可以绑定防DDoS Pro实例注意：此字段可能返回null，表示无法获得有效值",
			"config_id":                    "CLB实例级别的自定义配置ID注意：此字段可能返回null，表示无法获得有效值",
			"load_balancer_pass_to_target": "真实的服务器是否打开从CLB实例到互联网的流量注意：此字段可能返回null，表示无法获得有效值",
			"exclusive_cluster":            "专用网络群集注意：此字段可能返回null，表示无法获得有效值",
			"l4_clusters":                  "第4层专用集群列表注意：此字段可能返回null，表示无法获得有效值",
			"cluster_id":                   "唯一的群集ID",
			"cluster_name":                 "群集名称注意：此字段可能返回null，表示无法获得有效值",
			"l7_clusters":                  "第7层专用集群列表注意：此字段可能返回null，表示无法获得有效值",
			"classical_cluster":            "vpcgw集群注意：此字段可能返回null，表示无法获得有效值",
			"ipv6_mode":                    "只有当IP地址版本为ipv6时，此字段才有意义有效值：IPv6Nat64、IPv6FullChain注意：此字段可能返回null，表示无法获得有效值",
			"snat_pro":                     "是否启用SnatPro注意：此字段可能返回null，表示无法获得有效值",
			"snat_ips":                     "启用SnatPro负载平衡后的SnatIp列表注意：此字段可能返回null，表示无法获得有效值",
			"ip":                           "IP地址，例如192.168.0.1",
			"sla_type":                     "支持LCU的实例的规范注意：此字段可能返回null，表示无法获得有效值",
			"is_block":                     "VIP是否被阻止注意：此字段可能返回null，表示无法获得有效值",
			"is_block_time":                "时间被阻止或未被阻止注意：此字段可能返回null，表示无法获得有效值",
			"local_bgp":                    "IP类型是否为本地BGP",
			"cluster_tag":                  "专用的第7层标签注意：此字段可能返回null，表示无法获得有效值",
			"mix_ip_target":                "如果启用了IPv6FullChain CLB实例的第7层侦听器，则CLB实例可以同时与IPv4和IPv6 CVM实例绑定注意：此字段可能返回null，表示无法获得有效值",
			"zones":                        "基于专有网络的专用网络CLB实例的可用区注意：此字段可能返回null，表示无法获得有效值",
			"nfv_info":                     "是否为NFV CLB实例无返回信息：无；l7nfv：是的注意：此字段可能返回null，表示无法获得有效值",
			"health_log_set_id":            "CLB CLS的运行状况检查日志集ID注意：此字段可能返回null，表示无法获得有效值",
			"health_log_topic_id":          "CLB CLS的运行状况检查日志主题ID注意：此字段可能返回null，表示无法获得有效值",
			"cluster_ids":                  "集群ID注意：此字段可能返回null，表示无法获得有效值",
			"attribute_flags":              "集群ID注意：此字段可能返回null，表示无法获得有效值",
...
func dataSourceTencentCloudClbInstanceByCertId() *schema.Resource {
	return &schema.Resource{
		Description: "Use this data source to query detailed information of clb instance_by_cert_id",
