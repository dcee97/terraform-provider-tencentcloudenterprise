...
func init() {
	registerResourceDescriptionProvider("cloud_redis_instance", CNDescription{
		...
		AttributesCN: map[string]string{
			"availability_zone":   "可用区ID",
			"name":                "实例名称",
			"type_id":             "实例类型ID",
			"redis_shard_num":     "分片数",
			"redis_replicas_num":  "副本数",
			"replica_zone_ids":    "副本可用区ID",
			"type":                "实例类型",
			"password":            "密码",
			"no_auth":             "是否支持无密码访问",
			"replicas_read_only":  "是否支持副本只读",
			"mem_size":            "内存大小",
			"vpc_id":              "VPC ID",
			"subnet_id":           "子网ID",
			...
			"create_time":         "创建时间",
			"charge_type":         "计费类型",
			"prepaid_period":      "预付费时长",
			"force_delete":        "是否强制删除",
			"auto_renew_flag":     "自动续费标识",
			"operation_network":   "网络操作类型",
			"recycle":             "回收站保留时间",
			"ip":                  "IP地址",
			"tags":                "标签",
			"resource_pool_id":    "资源池ID",
			"platform_project_id": "平台项目ID",
		},
	})
}
func resourceTencentCloudRedisInstance() *schema.Resource {
	...
	return &schema.Resource{
		Description: "Provides a resource to create a instance and set its attributes.",
		...
		Schema: map[string]*schema.Schema{
			...
			"resource_pool_id": {
				...
				Optional:    true,
				ForceNew:    true,
				Description: "The ID of the resource pool to which the instance belongs.",
			},
			"platform_project_id": {
				Type:        schema.TypeString,
				Optional:    true,
				ForceNew:    true,
				Description: "The platform project ID is used to specify which project this resource belongs to.",
			},
			//"node_info": {
			//	Type:        schema.TypeList,
			//	Computed:    true,
...
func resourceTencentCloudRedisInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	var (
		...
		options          []CreateInstanceReqOption
	)
	...
	if v, ok := d.GetOk("resource_pool_id"); ok {
		options = append(options, WithResourcePool(v.(string)))
	}

	if v, ok := d.GetOk("platform_project_id"); ok {
		options = append(options, WithPlatformProjectID(v.(string)))
	}
	...
	//Comment: ---2023-08-08 专有云不支持NodeSet参数
...
func resourceTencentCloudRedisInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("mem_size") {
		...
		err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
			_, err := redisService.UpgradeInstance(ctx, id, newMemSize, redisShardNum, redisReplicasNum)
			if err != nil {
	...
	if d.HasChange("redis_shard_num") {
		...
		memSize := d.Get("mem_size").(int)
		err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
			_, err := redisService.UpgradeInstance(ctx, id, memSize, redisShardNum, redisReplicasNum)
			if err != nil {
				// Upgrade memory will cause instance lock and cannot acknowledge by polling status, wait until lock release
				return retryError(err, redis.FAILEDOPERATION_UNKNOWN, redis.FAILEDOPERATION_SYSTEMERROR)
			}
	...
	//		if e != nil {
	//			if _, ok := e.(*sdkErrors.CloudSDKError); !ok {
...
func resourceRedisNodeSetModify(ctx context.Context, service *RedisService, d *schema.ResourceData) error {
	...
	if len(adds) > 0 {
		...
		//}
		err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
			_, err := service.UpgradeInstance(ctx, d.Id(), memSize, shardNum, len(adds))
			if err != nil {
	...
	if len(lacks) > 0 {
		...
		//}
		replicasParam := len(lacks)
		err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
			_, err := service.UpgradeInstance(ctx, id, memSize, shardNum, replicasParam)
			if err != nil {
				return retryError(err, redis.FAILEDOPERATION_UNKNOWN)
			}
			return nil
