...
func init() {
	registerDataDescriptionProvider("cloud_vpc_instances", CNDescription{
		...
		AttributesCN: map[string]string{
			"vpc_id":             "要查询的专有网络的ID",
			"name":               "要查询的专有网络的名称",
			"is_default":         "过滤默认或无默认VPC",
			"tag_key":            "如果VPC有这个标签就过滤",
			"cidr_block":         "用这个CIDR过滤VPC",
			"tags":               "要查询的专有网络的标签",
			"result_output_file": "用于保存结果",
			"instance_list":      "专有网络的信息列表",
			"is_multicast":       "表示是否启用VPC组播",
			"dns_servers":        "VPC内可以使用的DNS服务器列表",
			"subnet_ids":         "此VPC内子网的ID列表",
			"create_time":        "VPC创建时间",
		},
	})
}
...
func dataSourceTencentCloudVpcInstances() *schema.Resource {
	return &schema.Resource{
		Description: "Query VPC instances information.",
