...
func init() {
	registerDataDescriptionProvider("cloud_as_last_activity", CNDescription{
		...
		AttributesCN: map[string]string{
			"auto_scaling_group_ids":        "自动缩放组的ID列表",
			"activity_set":                  "符合条件的扩展活动的信息集不返回没有缩放活动的缩放组例如，如果有50个自动缩放组ID，但只返回45条记录，则表明5个自动缩放的组没有缩放活动",
			"auto_scaling_group_id":         "自动缩放组ID",
			"activity_id":                   "缩放活动ID",
			"activity_type":                 "缩放活动的类型值范围：SCALE_OUT、SCALE_IN、ATTACH_INSTANCES、REMOVE_INSTANCES、DETACH_STANCES，TERMINATE_INSTANCES_UNEXPECTEDLY、REPLACE_UNHEALTHY_INSTANCE、START_INSTANCES，STOP_INSTANCES和INVOKE_COMMAND",
			"status_code":                   "缩放活动状态值范围：INIT、RUNNING、SUCCESSFUL、PARTIALLY_SUCCESSFUL、FAILED、CANCELLED",
			"status_message":                "缩放活动状态的描述",
			"cause":                         "缩放活动的原因",
			"description":                   "缩放活动的描述",
			"start_time":                    "缩放活动的开始时间",
			"end_time":                      "缩放活动的结束时间",
			"created_time":                  "缩放活动的创建时间",
			"activity_related_instance_set": "与缩放活动相关的实例的信息集",
			"instance_id":                   "实例ID",
			"instance_status":               "缩放活动中实例的状态取值范围：INIT、RUNNING、SUCCESSFUL、FAILED",
			"status_message_simplified":     "缩放活动状态的简要说明",
			"lifecycle_action_result_set":   "缩放活动中生命周期挂钩操作的结果",
			"lifecycle_hook_id":             "生命周期挂钩的ID",
			"invocation_id":                 "执行任务ID，可以通过TAT的DescribeInvocations API查询结果",
			"invoke_command_result":         "命令调用的结果，值范围：SUCCESSFUL、FAILED、NONE",
			"notification_result":           "通知结果，表示通知CMQ/TDMQ是否成功，取值范围：successful、FAILED、NONE",
			"lifecycle_action_result":       "生命周期挂钩操作的结果，值范围：CONTINUE，ABANDON",
			"result_reason":                 "结果原因，值范围：HEARTBEAT_TIMEOUT：检测信号超时使用DefaultResult的设置NOTIFICATION_FAILURE:无法发送通知使用DefaultResult的设置CALL_INTERFACE：调用CompleteLifecycleAction来设置结果ANOTHER_ACTION_ABANDON:它已被另一个操作设置为ABANDONCOMMAND_CALL_FAILURE:无法调用该命令将应用DefaultResultCOMMAND_EXEC_FINISH:命令已完成COMMAND_CALL_FAILURE:无法执行该命令将应用DefaultResultCOMMAND_EXEC_RESULT_CHECK_FAILURE：无法检查命令结果将应用DefaultResult",
			"detailed_status_message_set":   "缩放活动状态的详细描述",
			"code":                          "错误类型",
			"zone":                          "AZ信息",
			"instance_charge_type":          "实例计费模式",
			"subnet_id":                     "子网ID",
			"message":                       "错误消息",
			"instance_type":                 "实例类型",
			"invocation_result_set":         "命令执行的结果",
			"invocation_task_id":            "执行任务ID注意：此字段可能返回null，表示无法获得有效值",
			"command_id":                    "命令ID注意：此字段可能返回null，表示无法获得有效值",
			"task_status":                   "执行状态注意：此字段可能返回null，表示无法获得有效值",
			"error_message":                 "执行异常信息注意：此字段可能返回null，表示无法获得有效值",
...
func dataSourceTencentCloudAsLastActivity() *schema.Resource {
	return &schema.Resource{
		Description: "Use this data source to query detailed information of as last_activity",
