...
func init() {
	registerDataDescriptionProvider("cloud_vpc_subnets", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"subnet_id":          "要查询的子网的ID",
			"name":               "要查询的子网的名称",
			"availability_zone":  "要查询的子网的区域",
			"is_default":         "筛选默认或不筛选默认子网",
			"tag_key":            "如果子网有此标记,则进行筛选",
			"cidr_block":         "使用此CIDR筛选子网",
			"is_remote_vpc_snat": "过滤VPC SNAT地址池子网",
			"tags":               "要查询的子网的标记",
			"result_output_file": "用于保存结果",
			"instance_list":      "子网列表",
			"is_multicast":       "指示是否启用了多播",
			"route_table_id":     "路由表的ID",
			"available_ip_count": "可用IP的数量",
			"create_time":        "子网资源的创建时间",
		},
	})
}
