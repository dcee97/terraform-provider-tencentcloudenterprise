...
func (me *ClbService) DescribeAttachmentByPara(ctx context.Context, clbId string, listenerId string, locationId string) (clbAttachment *clb.ListenerBackend, errRet error) {
	...
	if aErr != nil {
		//in case that the lb is not exist, return empty
		if e, ok := aErr.(*sdkErrors.CloudSDKError); ok {
			if e.GetCode() == "InvalidParameter.LBIdNotFound" {
				return
			}
		}
...
func (me *ClbService) DeleteAttachmentById(ctx context.Context, clbId string, listenerId string, locationId string, targets []interface{}) error {
	...
	if err != nil {
		ee, ok := err.(*sdkErrors.CloudSDKError)
...
func checkHealthCheckPara(ctx context.Context, d *schema.ResourceData, protocol string, applyType string) (healthSetFlag bool, healthCheckPara *clb.HealthCheck, errRet error) {
	...
	if v, ok := d.GetOk("health_check_context_type"); ok {
		if !((protocol == CLB_LISTENER_PROTOCOL_UDP || protocol == CLB_LISTENER_PROTOCOL_TCP) &&
			checkType == HEALTH_CHECK_TYPE_CUSTOM) {
			healthSetFlag = false
...
func checkCertificateInputPara(ctx context.Context, d *schema.ResourceData, meta interface{}) (certificateSetFlag bool, certPara *clb.CertificateInput, errRet error) {
	...
	if certificateInput.CertCaId != nil {
		check, err := clbService.checkCertificateType(ctx, *certificateInput.CertCaId, SSL_CERT_TYPE_CA)
		if err != nil {
			certificateSetFlag = false
		...
		if !check {
			certificateSetFlag = false
			errRet = fmt.Errorf("certificated %s check error cert type is not `%s`",
				*certificateInput.CertCaId, SSL_CERT_TYPE_CA)
			return
		}
	}
	if certificateInput.CertId != nil {
		check, err := clbService.checkCertificateType(ctx, *certificateInput.CertId, SSL_CERT_TYPE_SERVER)
		if err != nil {
			certificateSetFlag = false
...
func clbNewTarget(instanceId, eniIp, port, weight, instance_bms_id interface{}) *clb.Target {
	p := int64(port.(int))
	target := clb.Target{
		Port: &p,
	}
	...
	if id, ok := instance_bms_id.(string); ok && id != "" {
		target.InstanceBmsId = &id
	}
	return &target
}
...
func (me *ClbService) DescribeClbInstanceDetailByFilter(ctx context.Context,
	param map[string]interface{}) (instanceDetail []*clb.LoadBalancer, errRet error) {
	var (
		logId   = getLogId(ctx)
		request = clb.NewDescribeLoadBalancersRequest()
	)
	...
	for {
		...
		instanceDetail = append(instanceDetail, response.Response.LoadBalancerSet...)
...
func (me *ClbService) DescribeClbResourcesByFilter(ctx context.Context, param map[string]interface{}) (
	resources []*clb.LoadBalancer, errRet error) {
	var (
		logId   = getLogId(ctx)
		request = clb.NewDescribeLoadBalancersRequest()
	)
	...
	for {
		...
		resources = append(resources, response.Response.LoadBalancerSet...)
		if len(response.Response.LoadBalancerSet) < int(limit) {
			break
		}
...
func (me *ClbService) SetClbSecurityGroup(ctx context.Context,
	securityGroup string, lbId string, operation string, lbIds []*string) (errRet error) {
	logId := getLogId(ctx)

	request := clb.NewSetSecurityGroupForLoadbalancersRequest()
	request.SecurityGroup = &securityGroup
	request.LoadBalancerIds = lbIds
...
func (me *ClbService) UploadCertificate(ctx context.Context, request *clb.UpLoadCertRequest) (id string,
	err error) {
	logId := getLogId(ctx)
	client := me.client.UseClbClient()
	ratelimit.Check(request.GetAction())

	var response *clb.UpLoadCertResponse
	response, err = client.UpLoadCert(request)
	if err != nil {
	...
	if response.Response.CertID == nil {
		err = fmt.Errorf("api[%s] return id is nil", request.GetAction())
		log.Printf("[CRITAL]%s %v", logId, err)
		return
	}

	id = *response.Response.CertID
	return
}

func (me *ClbService) DeleteCertificate(ctx context.Context, request *clb.DeleteCertRequest) (
	err error) {
	logId := getLogId(ctx)
	client := me.client.UseClbClient()
	ratelimit.Check(request.GetAction())

	var response *clb.DeleteCertResponse

	response, err = client.DeleteCert(request)
	if err != nil {
		log.Printf("[CRITAL]%s api[%s] fail, request body [%s], reason[%s]\n",
			logId, request.GetAction(), request.ToJsonString(), err.Error())
		return
...
func (me *ClbService) DescribeCert(ctx context.Context, request *clb.DescribeCertsRequest) (
	certs []*clb.CertList, err error) {
	logId := getLogId(ctx)
	client := me.client.UseClbClient()
	ratelimit.Check(request.GetAction())
	...
	for {
		...
		response, e := client.DescribeCerts(request)
		if e != nil {
			log.Printf("[CRITAL]%s api[%s] fail, request body [%s], reason[%s]\n",
				logId, request.GetAction(), request.ToJsonString(), err.Error())
			err = e
		...
		certs = append(certs, response.Response.CertList...)
		if len(response.Response.CertList) < pageSize {
			break
		}
		offset += pageSize
	}
	...
	// 	return
	// }
	// certs = response.Response.CertList
	return
}

func (me *ClbService) ReplaceCert(ctx context.Context, request *clb.ReplaceCertRequest) (
	response *clb.ReplaceCertResponse, err error) {
	logId := getLogId(ctx)
	client := me.client.UseClbClient()
	ratelimit.Check(request.GetAction())

	response, err = client.ReplaceCert(request)
	if err != nil {
		log.Printf("[CRITAL]%s api[%s] fail, request body [%s], reason[%s]\n",
			logId, request.GetAction(), request.ToJsonString(), err.Error())
		return
...
func (me *ClbService) checkCertificateType(ctx context.Context, certId string, checkType string) (bool, error) {
	//get certificate by id
	request := clb.NewDescribeCertsRequest()
	request.CertIdList = []*string{&certId}
	certificate, err := me.DescribeCert(ctx, request)
	if err != nil {
		return false, err
	}
	if len(certificate) < 1 {
		return false, fmt.Errorf("cert id %s not found", certId)
	}

	if *certificate[0].CertType != checkType {
		return false, fmt.Errorf("cert type %s is not correct", checkType)
	}
	return true, nil
}
