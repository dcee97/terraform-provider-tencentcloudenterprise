...
func init() {
	registerDataDescriptionProvider("cloud_vpc_nat_gateways", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"nats":               "专用NAT的信息列表",
			"state":              "NAT网关的状态",
			"max_concurrent":     "NAT网关并发连接的上限，可用值包括：100000300000010000000默认值为1000000",
			"bandwidth":          "NAT网关的最大公网输出带宽（单位：Mbps），可用值包括：20,50100200500100020005000默认值为100",
			"assigned_eip_set":   "绑定到网关的EIP IP地址集值至少为1",
			"create_time":        "NAT网关的创建时间",
			"tags":               "此NAT网关内的可用标记",
		},
	})
}
...
func dataSourceTencentCloudNatGateways() *schema.Resource {
	return &schema.Resource{
		Description: "Query NAT gateways",
