...
func init() {
	registerResourceDescriptionProvider("cloud_tsf_application", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"application_type":               "应用类型, V: 虚拟机, C: 容器",
			"microservice_type":              "应用微服务类型, N: 普通应用, G: 网关应用, M: Service Mesh应用",
			"application_desc":               "应用描述",
			"application_log_config":         "应用日志配置",
			"application_resource_type":      "应用资源类型",
			"application_runtime_type":       "应用运行时类型",
			"program_id":                     "绑定的数据集ID",
			"service_config_list":            "服务配置信息",
			"ignore_create_image_repository": "是否忽略创建镜像仓库",
			"program_id_list":                "N/A",
			"health_check":                   "健康检查配置",
			"name":                           "服务名称",
			"path":                           "健康检查路径",
...
func resourceTencentCloudTsfApplication() *schema.Resource {
	return &schema.Resource{
		Description: "Provides a resource to create a tsf application",
		...
		Schema: map[string]*schema.Schema{
			...
			"application_type": {
				Required:     true,
				Type:         schema.TypeString,
				ValidateFunc: validateAllowedStringValue([]string{"V", "C"}),
				Description:  "Application type: V for virtual machine, C for container.",
			},

			"microservice_type": {
				Required:     true,
				Type:         schema.TypeString,
				ValidateFunc: validateAllowedStringValue([]string{"N", "G", "M"}),
...
func resourceTencentCloudTsfApplicationCreate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.cloud_tsf_application.create")()
...
func resourceTencentCloudTsfApplicationRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.cloud_tsf_application.read")()
	defer inconsistentCheck(d, meta)()
...
func resourceTencentCloudTsfApplicationUpdate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.cloud_tsf_application.update")()
	defer inconsistentCheck(d, meta)()
...
func resourceTencentCloudTsfApplicationDelete(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.cloud_tsf_application.delete")()
	defer inconsistentCheck(d, meta)()
