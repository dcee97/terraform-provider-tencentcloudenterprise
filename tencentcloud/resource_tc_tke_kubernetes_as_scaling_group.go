...
func init() {
	registerResourceDescriptionProvider("cloud_tke_kubernetes_as_scaling_group", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"load_balancer_ids":         "传统负载均衡器ID列表, LoadBalancerIds 和 ForwardLoadBalancers 二者同时最多只能指定一个",
			"forward_balancer_ids":      "应用负载均衡器ID列表, LoadBalancerIds 和 ForwardLoadBalancers 二者同时最多只能指定一个",
			"termination_policies":      "销毁策略",
			"retry_policy":              "重试策略",
			"tags":                      "标签列表",
			"auto_scaling_config":       "伸缩组配置参数",
