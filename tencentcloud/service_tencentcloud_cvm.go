...
func (me *CvmService) ModifyProjectId(ctx context.Context, instanceId string, projectId string) error {
	logId := getLogId(ctx)
...
func (me *CvmService) DescribeInstancesSellTypeByFilter(ctx context.Context, filters map[string][]string) (instanceTypes []*cvm.InstanceTypeQuotaItemArchitecture, errRet error) {
	logId := getLogId(ctx)
...
func (me *CvmService) CreatePlacementGroup(ctx context.Context, placementName, placementType,
	strategy string) (placementId string, errRet error) {
	logId := getLogId(ctx)
...
func (me *CvmService) DescribePlacementGroupByFilter(ctx context.Context, id, name string, strategies []*string) (placementGroups []*cvm.DisasterRecoverGroup, errRet error) {
	logId := getLogId(ctx)
	request := cvm.NewDescribeDisasterRecoverGroupsRequest()
	if id != "" {
...
func (me *CvmService) DescribeCvmLaunchTemplateById(ctx context.Context,
	launchTemplateId string) (launchTemplate *cvm.LaunchTemplatesInfo, errRet error) {
	logId := getLogId(ctx)

	request := cvm.NewDescribeLaunchTemplatesInfoRequest()
	...
	response, err := me.client.UseCvmClient().DescribeLaunchTemplatesInfo(request)
	if err != nil {
		errRet = err
		return
	}
	...
	if len(response.Response.LaunchTemplateInfoSet) < 1 {
		return
	}

	launchTemplate = response.Response.LaunchTemplateInfoSet[0]
	return
}
...
func (me *CvmService) DeleteCvmLaunchTemplateById(ctx context.Context, launchTemplateId string) (errRet error) {
	...
	request := cvm.NewDeleteLaunchTemplatesRequest()
	request.LaunchTemplateIds = []*string{&launchTemplateId}
	...
	response, err := me.client.UseCvmClient().DeleteLaunchTemplates(request)
	if err != nil {
		errRet = err
		return
	}
...
func (me *CvmService) CreateKeyPairByImportPublicKey(ctx context.Context, projectId int64, keyName,
	publicKey string) (keyId string, err error) {
	logId := getLogId(ctx)
	request := cvm.NewImportKeyPairRequest()
	response := cvm.NewImportKeyPairResponse()
	ratelimit.Check(request.GetAction())
	request.KeyName = helper.String(keyName)
	request.ProjectId = helper.Int64(projectId)
	request.PublicKey = helper.String(publicKey)

	innerErr := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		result, e := me.client.UseCvmClient().ImportKeyPair(request)
		if e != nil {
			return retryError(e)
		} else {
			log.Printf("[DEBUG]%s api[%s] success, request body [%s], response body [%s]\n", logId, request.GetAction(), request.ToJsonString(), result.ToJsonString())
