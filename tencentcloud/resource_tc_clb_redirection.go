...
func resourceTencentCloudClbRedirection() *schema.Resource {
	return &schema.Resource{
		Description: "Provides a resource to create a CLB redirection.",
		...
		Schema: map[string]*schema.Schema{
			...
			"is_auto_rewrite": {
				...
				ForceNew: true,
				Optional: true,
				Description: "Indicates whether automatic forwarding is enable, default is `false`." +
					" If enabled, the source listener and location will not be used, the target listener and location must be https protocol and port is 443.",
			},
			"delete_all_auto_rewrite": {
				Type:     schema.TypeBool,
				Default:  false,
				Optional: true,
				Description: "Indicates whether delete all auto redirection. Default is `false`. " +
					"It will take effect only when this redirection is auto-rewrite and this auto-rewrite auto redirected more than one rules. " +
					"All the auto-rewrite relations will be deleted when this parameter set true.",
			},
		},
	}
}
...
func resourceTencentCloudClbRedirectionCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if isAutoRewrite {
		...
		// 未配置auto rewrite，则从入参中获取source相关参数
		if v, ok := d.GetOk("source_listener_id"); ok {
			sourceListenerId = v.(string)
			checkErr = ListenerIdCheck(sourceListenerId)
			if checkErr != nil {
