...
func init() {
	registerResourceDescriptionProvider("cloud_tsf_path_rewrite", CNDescription{
		...
		AttributesCN: map[string]string{
			"path_rewrite_id":  "路径重写规则ID",
			"gateway_group_id": "网关部署组ID",
			"regex":            "正则表达式",
			"replacement":      "替换内容",
			"blocked":          "是否屏蔽映射路径，Y：是 N：否",
			"order":            "规则顺序，值越小优先级越高",
		},
	})
}
...
func resourceTencentCloudTsfPathRewrite() *schema.Resource {
	return &schema.Resource{
		Description: "Provides a resource to create a tsf path_rewrite",
...
func resourceTencentCloudTsfPathRewriteCreate(d *schema.ResourceData, meta interface{}) error {
	...
	gatewayGroupId := d.Get("gateway_group_id").(string)
	pathRewrite.GatewayGroupId = helper.String(gatewayGroupId)
