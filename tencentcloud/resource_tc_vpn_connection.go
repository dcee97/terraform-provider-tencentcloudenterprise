...
func init() {
	registerResourceDescriptionProvider("cloud_vpn_connection", CNDescription{
		...
		AttributesCN: map[string]string{
			"name":                       "VPN通道名称",
			"vpc_id":                     "VPC实例ID",
			"customer_gateway_id":        "对端网关ID",
			"vpn_gateway_id":             "VPN网关ID",
			"pre_share_key":              "预共享密钥",
			"security_group_policy":      "安全组策略",
			"ike_proto_encry_algorithm":  "IKE协议加密算法",
			"ike_proto_authen_algorithm": "IKE协议认证算法",
			"ike_exchange_mode":          "IKE协议模式",
			"ike_local_identity":         "IKE本端标识类型",
			"ike_remote_identity":        "IKE对端标识类型",
			"ike_local_address":          "IKE本端标识地址",
			"ike_remote_address":         "IKE对端标识地址",
			"ike_local_fqdn_name":        "IKE本端标识FQDN",
			"ike_remote_fqdn_name":       "IKE对端标识FQDN",
			"ike_dh_group_name":          "IKE协议DH组",
			"ike_sa_lifetime_seconds":    "IKE协议SA生存周期",
			"ike_version":                "IKE协议版本",
			"ipsec_encrypt_algorithm":    "IPSEC协议加密算法",
			"ipsec_integrity_algorithm":  "IPSEC协议认证算法",
			"ipsec_sa_lifetime_seconds":  "IPSEC协议SA生存周期",
			"ipsec_pfs_dh_group":         "IPSEC协议PFS DH组",
			"ipsec_sa_lifetime_traffic":  "IPSEC协议SA生存周期",
			"tags":                       "标签列表",
			"dpd_enable":                 "是否开启健康检查",
			"dpd_timeout":                "健康检查超时时间",
			"dpd_action":                 "健康检查超时后的处理动作",
			"create_time":                "创建时间",
			"vpn_proto":                  "VPN协议",
			"encrypt_proto":              "加密协议",
			"route_type":                 "路由类型",
			"state":                      "通道状态",
			"net_status":                 "通道连接状态",
			"enable_health_check":        "是否开启健康检查",
			"health_check_local_ip":      "本端健康检查地址",
			"health_check_remote_ip":     "对端健康检查地址",
		},
	})
}
...
func resourceTencentCloudVpnConnectionDelete(d *schema.ResourceData, meta interface{}) error {
	...
	err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		...
		if e != nil {
			if ee, ok := e.(*errors.CloudSDKError); ok {
