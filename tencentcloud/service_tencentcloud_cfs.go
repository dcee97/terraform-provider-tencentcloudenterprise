...
func (me *CfsService) DescribeCfsAutoSnapshotPolicyAttachmentById(ctx context.Context, autoSnapshotPolicyId string, fileSystemIds string) (autoSnapshotPolicyAttachment *cfs.AutoSnapshotPolicyInfo, errRet error) {
	...
	fileSystemMap := make(map[string]bool)
	for _, sysID := range fileSystemIdsList {
		fileSystemMap[sysID] = true
	}

	for _, resID := range res {
		if !fileSystemMap[resID] {
			return
		}
	}
	autoSnapshotPolicyAttachment = autoSnapshotPolicy
...
func (me *CfsService) DescribeCfsSnapshotById(ctx context.Context, jobId string) (snapshot *cfs.SnapList,
	errRet error) {
	logId := getLogId(ctx)

	request := cfs.NewDescribeCfsSnapsRequest()
	request.JobId = &jobId
	...
	instances := make([]*cfs.SnapList, 0)
	for {
		request.Offset = &offset
		request.Limit = &limit
		response, err := me.client.UseCfsClient().DescribeCfsSnaps(request)
		if err != nil {
			errRet = err
			return
		}
...
func (me *CfsService) DeleteCfsSnapshotById(ctx context.Context, snapshotId string) (errRet error) {
	...
	request := cfs.NewDeleteCfsSnapRequest()
	request.SnapId = &snapshotId
	...
	response, err := me.client.UseCfsClient().DeleteCfsSnap(request)
	if err != nil {
		errRet = err
		return
	}
...
func (me *CfsService) CfsSnapshotStateRefreshFunc(jobId string, failStates []string) resource.StateRefreshFunc {
