...
func init() {
	registerDataDescriptionProvider("cloud_as_scaling_policies", CNDescription{
		...
		AttributesCN: map[string]string{
			"scaling_policy_id":           "缩放策略ID",
			"scaling_group_id":            "缩放组ID",
			"policy_name":                 "缩放策略名称",
			"result_output_file":          "用于保存结果",
			"scaling_policy_list":         "缩放策略列表每个元素都包含以下属性：",
			"adjustment_type":             "缩放规则的调整类型",
			"adjustment_缩放规则的调整值":         "缩放规则的调整值",
			"comparison_operator":         "比较运算符",
			"metric_name":                 "指示器的名称",
			"threshold":                   "报警阈值",
			"period":                      "时间段（秒）",
			"continuous_time":             "重试次数",
			"statistic":                   "统计类型",
			"cooldown":                    "缩放规则的冷却时间",
			"notification_user_group_ids": "触发警报时需要通知用户",
		},
	})
}
...
func dataSourceTencentCloudAsScalingPolicies() *schema.Resource {
	return &schema.Resource{
		Description: "Use this data source to query detailed information of scaling policy.",
