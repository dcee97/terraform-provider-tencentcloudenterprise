...
func init() {
	registerResourceDescriptionProvider("cloud_vpc_route_table_entry", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"destination_cidr_block": "目的IPv4网段",
			"next_type":              "下一跳类型",
			"next_hop":               "下一跳网关",
			"disabled":               "是否启用该条目",
			"description":            "路由表条目描述",
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"next_type": {
				...
				Description:  "Type of next-hop. Valid values: `VPN`, `DIRECTCONNECT`, `PEERCONNECTION`, `HAVIP`, `NAT`, `NORMAL_CVM`, `EIP` .",
			},
			"next_hop": {
				Type:        schema.TypeString,
				Required:    true,
...
func resourceTencentCloudVpcRouteEntryCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if temp, ok := d.GetOk("next_hop"); ok {
		nextHub = temp.(string)
	}
	...
	if nextType == GATE_WAY_TYPE_EIP && nextHub != "0" {
		return fmt.Errorf("if next_type is %s, next_hop can only be \"0\" ", GATE_WAY_TYPE_EIP)
	}
