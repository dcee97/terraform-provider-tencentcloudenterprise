...
func dataSourceTencentCloudDcGatewayInstancesRead(d *schema.ResourceData, meta interface{}) error {
	...
	var infos, err = service.DescribeDirectConnectGateways(ctx, id)
	if err != nil {
		return err
	}
	...
	for _, item := range infos {
		...
		infoMap["dcg_id"] = item.DirectConnectGatewayId
		infoMap["name"] = item.DirectConnectGatewayName
		infoMap["dcg_ip"] = item.DirectConnectGatewayIp
		...
		infoMap["cnn_route_type"] = item.CcnRouteType
