...
// if cause is *sdkErrors.CloudSDKError, will use sdk error request-id
func WrapErrorf(cause error, id, requestId, msg string, args ...interface{}) error {
	if cause == nil {
		return nil
	}

	if sdkErr, ok := cause.(*sdkErrors.CloudSDKError); ok && requestId == "" {
...
func UnwarpSDKError(err error) *sdkErrors.CloudSDKError {
	var result *sdkErrors.CloudSDKError
