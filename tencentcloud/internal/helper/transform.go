...
func Int64sToStringsPoint(int64s []int64) []*string {
	vs := make([]*string, 0, len(int64s))
	for _, v := range int64s {
		vs = append(vs, Int64ToStrPoint(v))
	}
	return vs
}

func InterfacesToInt64(configured []interface{}) []int64 {
	vs := make([]int64, 0, len(configured))
	for _, v := range configured {
		vs = append(vs, v.(int64))
	}
	return vs
}
