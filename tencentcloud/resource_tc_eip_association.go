...
func init() {
	registerResourceDescriptionProvider("cloud_eip_association", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"instance_id":          "要绑定的实例 ID，实例 ID 形如：`ins-11112222`，与 `NetworkInterfaceId` 不可同时指定",
			"network_interface_id": "要绑定的弹性网卡 ID，弹性网卡 ID 形如：`eni-11112222`，与 `InstanceId` 不可同时指定",
			"private_ip":           "要绑定的内网 IP，必须确保指定的 `PrivateIpAddress` 是指定的 `NetworkInterfaceId` 上的一个内网 IP",
		},
	})
}
...
func resourceTencentCloudEipAssociation() *schema.Resource {
	return &schema.Resource{
		Description: "Provides an eip resource associated with other resource like CVM, ENI and CLB.",
