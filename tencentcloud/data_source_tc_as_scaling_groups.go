...
func init() {
	registerDataDescriptionProvider("cloud_as_scaling_groups", CNDescription{
		...
		AttributesCN: map[string]string{
			"scaling_group_id":         "用于查询的指定缩放组ID",
			"configuration_id":         "按启动配置ID筛选结果",
			"scaling_group_name":       "用于查询的缩放组名称",
			"tags":                     "用于查询的标记",
			"result_output_file":       "用于保存结果",
			"scaling_group_list":       "缩放组的列表每个元素都包含以下属性：",
			"max_size":                 "CVM实例的最大数目",
			"min_size":                 "CVM实例的最小数目",
			"vpc_id":                   "与实例关联的vpc的ID",
			"project_id":               "缩放组所属项目的ID默认值为0",
			"subnet_ids":               "子网ID列表",
			"zones":                    "可用区域的列表",
			"default_cooldown":         "缩放组的默认冷却时间",
			"desired_capacity":         "所需数量的CVM实例",
			"load_balancer_ids":        "CVM实例附加到的传统clb id的列表",
			"forward_balancer_ids":     "应用程序clb id的列表",
			"load_balancer_id":         "可用负载平衡器的ID",
			"listener_id":              "应用程序负载平衡器的侦听器ID",
			"location_id":              "转发规则的ID",
			"target_attribute":         "目标规则的属性列表",
			"port":                     "端口号",
			"weight":                   "重量",
			"termination_policies":     "用于从扩展组中选择要终止的CVM实例的策略",
			"retry_policy":             "创建失败时可以使用重试策略",
			"status":                   "缩放组的当前状态",
			"instance_count":           "实例数",
			"create_time":              "创建AS组的时间",
			"multi_zone_subnet_policy": "多区域或子网策略，有效值：PRIORITY和EQUALITY",
		},
	})
}
...
func dataSourceTencentCloudAsScalingGroups() *schema.Resource {
	return &schema.Resource{
		Description: "This data source provides the detail information of an existing autoscaling group.",
...
func dataSourceTencentCloudAsScalingGroupRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.cloud_as_scaling_groups.read")()
