...
func init() {
	registerDataDescriptionProvider("cloud_dc_instances", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"instance_list":               "DC的信息列表",
			"state":                       "DC的状态，可用值包括“REJECTED”、“TOPAY”、“PAID”、“ALLOCATED”、“available”、“DELETING”和“DELETED”",
			"access_point_id":             "tne DC的接入点ID",
			...
			"customer_address":            "在客户端内互连DC的IP注意：此字段可能返回null，表示没有采用有效值",
			"customer_name":               "DC的申请人名称，默认值从帐户中获取注意：此字段可能返回null，表示没有采用有效值",
			"customer_email":              "DC的申请人电子邮件，默认从帐户中获取注意：此字段可能返回null，表示没有采用有效值",
			"customer_phone":              "DC的申请人电话号码，默认从帐户中获取注意：此字段可能返回null，表示没有采用有效值",
			"fault_report_contact_person": "报告故障的联系人注意：此字段可能返回null，表示没有采用有效值",
			"fault_report_contact_phone":  "报告故障的电话号码注意：此字段可能返回null，表示没有采用有效值",
			"enabled_time":                "启用资源的时间",
			"create_time":                 "资源的创建时间",
			"expired_time":                "资源的过期日期",
		},
	})
}
...
func dataSourceTencentCloudDcInstances() *schema.Resource {
	return &schema.Resource{
		Description: "Use this data source to query detailed information of DC instances.",
