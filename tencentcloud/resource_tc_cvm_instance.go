...
func init() {
	...
	registerResourceDescriptionProvider("cloud_cvm_instance", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"instance_charge_type_prepaid_period": "实例预付费周期",
			"instance_charge_type_prepaid_renew_flag": "实例预付费自动续费标志",
			"cdh_instance_type":                       "CDH实例类型",
			"cdh_host_id":                             "CDH实例ID",
			"internet_charge_type":                    "网络计费类型",
			"internet_max_bandwidth_out":              "公网出带宽",
			"allocate_public_ip":                      "分配公网IP",
			"vpc_id":                                  "VPC ID",
			"subnet_id":                               "子网ID",
			"security_groups":                         "安全组（已弃用，用orderly_security_groups代替，与orderly_security_groups不能同时设置）",
			"tags":                                    "标签",
			...
			"system_disk_id":                          "系统盘ID",
			"disable_security_service":                "禁用安全服务",
			"keep_image_login":                        "保留镜像登录",
			"disable_monitor_service":                 "禁用监控服务",
			"user_data":                               "用户数据，base64编码",
			"user_data_raw":                           "用户数据原始， base64编码",
			"data_disks":                              "数据盘",
			...
			"expired_time":                            "过期时间",
			"instance_id":                             "实例ID",
			"data_disk_id":                            "用于初始化数据磁盘的数据磁盘ID。当数据磁盘类型为“LOCAL_BASIC”和“LOCAL_SSD”时，不支持磁盘id",
			"data_disk_size":                          "数据磁盘的大小，单位为GB",
			"data_disk_snapshot_id":                   "数据磁盘的快照ID。所选数据磁盘快照大小必须小于数据磁盘大小",
			"data_disk_type":                          "数据磁盘类型。有效值LOCAL_BASIC本地磁盘、LOCAL_SSD本地SSD磁盘、LOCAL_NVME本地NVME磁盘，在InstanceType中指定，LOCAL_PRO本地HDD磁盘，在Instance Type中指定、CLOUD_BASIC HDD云磁盘、CLOUD_PREMIUM高级云存储、CLOUD_SSD SSD、CLOUD_HSSD增强型SSD、CLOUD_TSSD巨大型SSD、CLAUD_BSSD平衡型SSD",
			"delete_with_instance":                    "决定是否使用实例删除磁盘（仅适用于具有“POSTAID_BY_HOUR”实例的“CLOUD_BASIC”、“CLOUD_SSD”和“CLOUD_PREMIUM”磁盘），默认值为true",
			"throughput_performance":                  "为数据磁盘增加额外性能。仅当磁盘类型为“CLOUD_TSSD”或“CLOUD_HSSD”时有效",
		},
	},
	)
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"instance_count": {
				...
				Description:  "(Optional, Int, *Deprecated*) Please use `count` instead. The number of instances to be purchased. Value range:[1,100]; default value: 1.",
			},
			"instance_name": {
				Type:         schema.TypeString,
				Optional:     true,
			...
			"resource_type": {
				...
				Optional:    true,
				Default:     "instance",
				Description: "resource type, default instance",
			},
			"running_flag": {
			...
			"security_groups": {
				...
				Description:   "(Optional, Set, `Deprecated`) Please use `orderly_security_groups` instead. A list of security group IDs to associate with.",
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"data_disk_type": {
							...
							Required:    true,
							ForceNew:    true,
							Description: "Data disk type. For more information about limits on different data disk types, see [Storage Overview](https://intl.cloud.com/document/product/213/4952). Valid values: LOCAL_BASIC: local disk, LOCAL_SSD: local SSD disk, LOCAL_NVME: local NVME disk, specified in the InstanceType, LOCAL_PRO: local HDD disk, specified in the InstanceType, CLOUD_BASIC: HDD cloud disk, CLOUD_PREMIUM: Premium Cloud Storage, CLOUD_SSD: SSD, CLOUD_HSSD: Enhanced SSD, CLOUD_TSSD: Tremendous SSD, CLOUD_BSSD: Balanced SSD.",
						},
						"data_disk_size": {
							Type:     schema.TypeInt,
							Required: true,
			...
			"tags": {
				Type:        schema.TypeMap,
				Optional:    true,
				Description: "A mapping of tags to assign to the resource. For tag limits, please refer to [Use Limits](https://intl.cloud.com/document/product/651/13354).",
			},
			"force_delete": {
				Type:        schema.TypeBool,
				Optional:    true,
...
func resourceTencentCloudInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if _, ok := d.GetOk("platform_project_id"); ok {
		request.ProjectSpecification = &cvm.ProjectSpecification{
			PlatformProjectId: helper.String(d.Get("platform_project_id").(string)),
			ResourceType:      helper.String(d.Get("resource_type").(string)),
		}
	}
	...
	if v, ok := d.GetOk("instance_charge_type"); ok {
		...
		if instanceChargeType == CVM_CHARGE_TYPE_CDHPAID {
			...
			if v, ok := d.GetOk("cdh_host_id"); ok {
				request.Placement.HostIds = []*string{helper.String(v.(string))}
			} else {
...
func resourceTencentCloudInstanceUpdate(d *schema.ResourceData, meta interface{}) (err error) {
	...
	if d.HasChange("project_id") {
		...
		err := cvmService.ModifyProjectId(ctx, instanceId, projectId)
		if err != nil {
			return err
		}
