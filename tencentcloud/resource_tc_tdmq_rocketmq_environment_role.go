...
func resourceTencentCloudTdmqRocketmqEnvironmentRole() *schema.Resource {
	return &schema.Resource{
		Description: "Provides a resource to create a tdmqRocketmq environment_role",
...
func resourceTencentCloudTdmqRocketmqEnvironmentRoleCreate(d *schema.ResourceData, meta interface{}) error {
	...
	var (
		request         = tdmqRocketmq.NewCreateRocketMQEnvironmentRoleRequest()
		clusterId       string
		roleName        string
	...
	err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		result, e := meta.(*TencentCloudClient).apiV3Conn.UseTdmqClient().CreateRocketMQEnvironmentRole(request)
		if e != nil {
			return retryError(e)
		} else {
...
func resourceTencentCloudTdmqRocketmqEnvironmentRoleUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	request := tdmqRocketmq.NewModifyRocketMQEnvironmentRoleRequest()
	...
	err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		result, e := meta.(*TencentCloudClient).apiV3Conn.UseTdmqClient().ModifyRocketMQEnvironmentRole(request)
		if e != nil {
			return retryError(e)
		} else {
			log.Printf("[DEBUG]%s api[%s] success, request body [%s], response body [%s]\n",
