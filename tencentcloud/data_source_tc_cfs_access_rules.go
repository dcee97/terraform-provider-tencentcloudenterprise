...
func init() {
	registerDataDescriptionProvider("cloud_cfs_access_rules", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"access_rule_id":     "用于查询的指定访问规则ID",
			"result_output_file": "用于保存结果",
			"access_rule_list":   "CFS访问规则的信息列表每个元素都包含以下属性：",
			"auth_client_ip":     "访问规则的允许IP",
			"rw_permission":      "读取和写入权限",
			"user_permission":    "访问用户的权限",
			"priority":           "访问规则的优先级",
		},
	})
}
...
func dataSourceTencentCloudCfsAccessRules() *schema.Resource {
	return &schema.Resource{
		Description: "This data source provides the detail information of CFS access rule.",
