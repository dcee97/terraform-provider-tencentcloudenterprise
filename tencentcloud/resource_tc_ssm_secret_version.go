...
func init() {
	registerResourceDescriptionProvider("cloud_ssm_secret_version", CNDescription{
...
func resourceTencentCloudSsmSecretVersion() *schema.Resource {
	return &schema.Resource{
		...
		Description: "Provides a resource to create and manage SSM secret version",
...
func resourceTencentCloudSsmSecretVersionDelete(d *schema.ResourceData, meta interface{}) error {
	...
	return resource.Retry(readRetryTimeout, func() *resource.RetryError {
		...
		if e != nil {
			if sdkError, ok := e.(*sdkErrors.CloudSDKError); ok && sdkError.Code == "ResourceNotFound" {
				return nil
			}
