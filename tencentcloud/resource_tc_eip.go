...
func init() {
	registerResourceDescriptionProvider("cloud_eip", CNDescription{
		...
		AttributesCN: map[string]string{
			"name":                       "EIP 名称",
			"type":                       "EIP 类型，有效值：`EIP`",
			"address_count":              "EIP 数量，有效值：1",
			"anycast_zone":               "anycast 区域，有效值：`ANYCAST_ZONE_GLOBAL` 和 `ANYCAST_ZONE_OVERSEAS`",
			"applicable_for_clb":         "指示 anycast EIP 是否可以关联到 CLB，该字段在版本 1.27.0 起已被弃用",
			"internet_service_provider":  "EIP 的网络服务提供商，有效值：`BGP`、`CMCC`、`CTCC` 和 `CUCC`",
			"internet_charge_type":       "EIP 的计费方式，有效值：`BANDWIDTH_PACKAGE`、`BANDWIDTH_POSTPAID_BY_HOUR` 和 `TRAFFIC_POSTPAID_BY_HOUR`",
			"prepaid_period":             "实例的计费周期，默认值：`1`，备注：当 `internet_charge_type` 为 `BANDWIDTH_PREPAID_BY_MONTH` 时，此字段必须设置",
			"auto_renew_flag":            "自动续费标志，取值：0-默认状态（手动续费）；1-自动续费；2-明确不自动续费，备注：仅支持预付费 EIP",
			"internet_max_bandwidth_out": "EIP 带宽限制，单位：Mbps",
			"tags":                       "EIP 的标签",
			"bandwidth_package_id":       "带宽包 ID，在 `internet_charge_type` 为 `BANDWIDTH_PACKAGE` 时设置",
			"public_ip":                  "弹性 IP 地址",
			"status":                     "EIP 的当前状态",
		},
	})
...
func resourceTencentCloudEip() *schema.Resource {
	return &schema.Resource{
		Description: "Provides an EIP resource.",
		...
		Schema: map[string]*schema.Schema{
			...
			"type": {
				...
				ValidateFunc: validateAllowedStringValue([]string{EIP_TYPE_EIP}),
				ForceNew:     true,

				Description: "The type of eip. Valid value:  `EIP`.",
			},
			"address_count": {
				Type:        schema.TypeInt,
				Optional:    true,
				Description: "The number of eip. Valid value: 1.",
			},
			"anycast_zone": {
				Type:         schema.TypeString,
				Optional:     true,
			...
			"internet_charge_type": {
				...
				Optional:     true,
				Computed:     true,
				ValidateFunc: validateAllowedStringValue([]string{"BANDWIDTH_PACKAGE", "BANDWIDTH_POSTPAID_BY_HOUR", "TRAFFIC_POSTPAID_BY_HOUR"}),
				Description:  "The charge type of eip. Valid values: `BANDWIDTH_PACKAGE`, `BANDWIDTH_POSTPAID_BY_HOUR`  and `TRAFFIC_POSTPAID_BY_HOUR`.",
			},
...
func resourceTencentCloudEipCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("address_count"); ok {
		request.AddressCount = helper.Int(v.(int))
	}
	...
	if d.HasChange("internet_max_bandwidth_out") {
		if v, ok := d.GetOk("internet_max_bandwidth_out"); ok {
			...
			err = resource.Retry(readRetryTimeout, func() *resource.RetryError {
				...
				if eip.Bandwidth != nil && *eip.Bandwidth == uint64(bandwidthOut) {
					return nil
				}
				return resource.RetryableError(fmt.Errorf("waiting for EIP bandwidth to be updated to %d", bandwidthOut))
			})
			if err != nil {
				return fmt.Errorf("error waiting for EIP bandwidth update: %s", err)
			}
		}
	}
