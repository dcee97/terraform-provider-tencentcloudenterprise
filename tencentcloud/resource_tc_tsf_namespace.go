...
func init() {
	registerResourceDescriptionProvider("cloud_tsf_namespace", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"namespace_resource_type": "命名空间资源类型",
			"namespace_type":          "命名空间类型",
			"namespace_id":            "命名空间ID",
			"is_ha_enable":            "是否开启高可用",
			"program_id":              "绑定的数据集ID",
			"program_id_list":         "绑定的数据集ID列表",
			"namespace_code":          "命名空间编码",
			"is_default":              "是否为默认命名空间",
...
func resourceTencentCloudTsfNamespace() *schema.Resource {
	return &schema.Resource{
		Description: "Provides a resource to create a tsf namespace",
