...
func init() {
	...
	registerDataDescriptionProvider("cloud_cvm_instance_types", CNDescription{
		...
		AttributesCN: map[string]string{
			"cpu_core_count":       "实例的 CPU 核心数",
			"gpu_core_count":       "实例的 GPU 核心数",
			"memory_size":          "实例内存容量，单位：GB",
			"availability_zone":    "CVM 实例所在的可用区与 `filter` 冲突",
			"filter":               "一个或多个名称/值对过滤器与 `availability_zone` 冲突",
			"exclude_sold_out":     "表示是否过滤售罄的实例类型，默认为 false",
			"result_output_file":   "用于保存结果的文件",
			"instance_types":       "CVM 实例信息列表，其中每个元素包含以下属性:",
			"instance_type":        "实例的类型",
			"family":               "实例的类型系列",
			"instance_charge_type": "实例的计费类型",
			"status":               "实例的销售状态",
			"name":                 "过滤器的名称有效值：`zone`, `instance-family` 和 `instance-charge-type`",
			"values":               "过滤器的值",
		},
	})
}
...
func dataSourceTencentCloudInstanceTypesRead(d *schema.ResourceData, meta interface{}) error {
	...
	var instanceSellTypes []*cvm.InstanceTypeQuotaItemArchitecture
	var errRet error
	var err error
