...
func init() {
	registerDataDescriptionProvider("cloud_clb_redirections", CNDescription{
		...
		AttributesCN: map[string]string{
			"clb_id":             "要查询的CLB的ID",
			"source_listener_id": "要查询的源侦听器的ID",
			"target_listener_id": "要查询的目标侦听器的ID",
			"source_rule_id":     "要查询的源侦听器的规则ID",
			"target_rule_id":     "要查询的目标侦听器的规则ID",
			"result_output_file": "用于保存结果",
			"redirection_list":   "云负载平衡器重定向配置的列表每个元素都包含以下属性：",
		},
	})
}
...
func dataSourceTencentCloudClbRedirections() *schema.Resource {
	return &schema.Resource{
		Description: "Use this data source to query detailed information of CLB redirections",
...
func dataSourceTencentCloudClbRedirectionsRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.cloud_clb_redirections.read")()
