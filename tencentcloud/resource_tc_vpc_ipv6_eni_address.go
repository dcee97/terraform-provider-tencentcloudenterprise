...
func init() {
	registerResourceDescriptionProvider("cloud_vpc_ipv6_eni_address", CNDescription{
		...
		AttributesCN: map[string]string{
			"vpc_id":               "VPC实例ID",
			"network_interface_id": "弹性网卡实例ID",
			"ipv6_addresses":       "IPv6地址",
			"address":              "`IPv6地址，格式为`3402:4e00:20:100:0:8cd9:2a67:71f3`",
			"address_id":           "`EIP实例ID，例如`eip-hxlqja90`",
			"description":          "说明",
			"is_wan_ip_blocked":    "公网IP是否被屏蔽",
			"primary":              "是否掌握“IP”",
			"state":                "`IPv6的“地址状态”为`PENDING`: pending, `MIGRATING`: migrating, `DELETING`: deleting, `AVAILABLE`: available.",
		},
	})
}
func resourceTencentCloudVpcIpv6EniAddress() *schema.Resource {
	return &schema.Resource{
		Description: "Provides a resource to create a vpc ipv6_eni_address",
