...
func init() {
	registerDataDescriptionProvider("cloud_vpn_gateways", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"id":                 "VPN网关的ID",
			"public_ip_address":  "VPN网关的公共ip地址",
			"zone":               "VPN网关的区域",
			"vpc_id":             "专有网络的ID",
			"tags":               "要查询的VPN网关的标签",
			"result_output_file": "用于保存结果",
			"gateway_list":       "专用网关的信息列表",
			"bandwidth":          "VPN网关的最大公网输出带宽（单位：Mbps）",
			"type":               "网关实例的类型",
			"state":              "VPN网关的状态",
			"prepaid_renew_flag": "标志指示是否续订",
			"charge_type":        "VPN网关的收费类型",
			"expired_time":       "当计费类型为“PREPAID”时，VPN网关的过期时间",
			"is_address_blocked": "指示ip地址是否被阻止",
			"new_purchase_plan":  "新采购计划",
			"restrict_state":     "限制VPN网关的状态",
			"create_time":        "创建VPN网关的时间",
		},
	})
}
...
func dataSourceTencentCloudVpnGateways() *schema.Resource {
	return &schema.Resource{
		Description: "Query detailed information of VPN gateways.",
...
func dataSourceTencentCloudVpnGatewaysRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.cloud_vpn_gateways.read")()
