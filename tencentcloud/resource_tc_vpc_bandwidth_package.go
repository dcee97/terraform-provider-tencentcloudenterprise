...
func init() {
	registerResourceDescriptionProvider("cloud_vpc_bandwidth_package", CNDescription{
		...
		AttributesCN: map[string]string{
			"network_type":           "带宽包类型",
			"charge_type":            "计费类型",
			"bandwidth_package_name": "带宽包名称",
			"internet_max_bandwidth": "带宽包大小",
			"tags":                   "标签",
		},
	})
}
func resourceTencentCloudVpcBandwidthPackage() *schema.Resource {
	return &schema.Resource{
		Description: "Provide a resource to create a vpc bandwidth_package",
...
func resourceTencentCloudVpcBandwidthPackageRead(d *schema.ResourceData, meta interface{}) error {
	...
	if bandwidthPackage == nil {
		d.SetId("")
		log.Printf("[WARN]%s resource `cloud_vpc_bandwidth_package` [%s] not found, please check if it has been deleted.",
