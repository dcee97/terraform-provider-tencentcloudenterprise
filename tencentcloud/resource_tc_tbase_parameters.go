/*
Provides a resource to create a Tbase parameters
...
func resourceTencentCloudTbaseParameters() *schema.Resource {
	return &schema.Resource{
		Read:   resourceTencentCloudTbaseParametersRead,
		Create: resourceTencentCloudTbaseParametersCreate,
		Update: resourceTencentCloudTbaseParametersUpdate,
		Delete: resourceTencentCloudTbaseParametersDelete,
...
func resourceTencentCloudTbaseParametersCreate(d *schema.ResourceData, meta interface{}) error {
	...
	d.SetId(instanceId)
	return resourceTencentCloudTbaseParametersRead(d, meta)
}
...
func resourceTencentCloudTbaseParametersRead(d *schema.ResourceData, meta interface{}) error {
	...
	parameters, err := service.DescribeDBParameters(ctx, &instanceId, nil)
...
func resourceTencentCloudTbaseParametersUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	//request := Tbase.NewModifyDBParametersRequest()
	//
	//instanceId := d.Id()
	//
	...
	//		dBParamValue := Tbase.DBParamValue{}
	...
	//	result, e := meta.(*TencentCloudClient).apiV3Conn.UseTbaseClient().ModifyDBParameters(request)
	//	if e != nil {
	//		return retryError(e)
	//	} else {
	...
	//
	//if err != nil {
	//	log.Printf("[CRITAL]%s create Tbase parameters failed, reason:%+v", logId, err)
	//	return err
	//}
