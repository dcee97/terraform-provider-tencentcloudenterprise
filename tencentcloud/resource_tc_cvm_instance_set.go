...
func init() {
	registerResourceDescriptionProvider("cloud_cvm_instance_set", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"key_name":                   "密钥ID",
			"password":                   "密码",
			"keep_image_login":           "保持镜像的原始设置。该参数与Password或KeyIds.N不能同时指定。只有使用自定义镜像、共享镜像或外部导入镜像创建实例时才能指定该参数为TRUE。",
			"private_ip":                 "私有IP",
			"user_data":                  "用户数据",
			"cam_role_name":              "CAM角色名称",
			"user_data_raw":              "用户数据原始内容，与UserData不能同时指定",
		},
	})
}
...
func resourceTencentCloudInstanceSet() *schema.Resource {
	return &schema.Resource{
		Description: "Provides a CVM instance set resource.",
...
func doResourceTencentCloudInstanceSetCreate(d *schema.ResourceData, meta interface{}) error {
	...
	//	request.ProjectSpecification.ResourceType = &resourceType
	//}
...
func doResourceTencentCloudInstanceSetUpdate(d *schema.ResourceData, meta interface{}) (err error) {
	...
	if d.HasChange("instance_count") {
		return fmt.Errorf("`instance_count` do not support change now, please use `resource_tc_cvm_instace` instead.")
	}
	...
	if d.HasChange("exclude_instance_ids") {
		...
		if len(needExclude) > 0 {
			...
			err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
				...
				if errRet != nil {
					...
					e, ok := errRet.(*sdkErrors.CloudSDKError)
					if ok && IsContains(CVM_RETRYABLE_ERROR, e.Code) {
						time.Sleep(1 * time.Second) // 需要重试的话，等待1s进行重试
		...
		if len(needCreate) > 0 {
			...
			//	platformProjectId := v.(string)
			//	request.ProjectSpecification.PlatformProjectId = &platformProjectId
			//}
