...
func init() {
	registerResourceDescriptionProvider("cloud_as_lifecycle_hook", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"lifecycle_transition":     "生命周期挂钩类型",
			"default_result":           "生命周期挂钩默认结果",
			"heartbeat_timeout":        "生命周期挂钩超时时间",
			"notification_metadata":    "生命周期挂钩通知元数据",
			"notification_target_type": "生命周期挂钩通知目标类型",
			"notification_queue_name":  "生命周期挂钩通知目标队列名称",
			"notification_topic_name":  "生命周期挂钩通知目标主题名称",
		},
	})
}
