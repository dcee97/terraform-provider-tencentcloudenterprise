...
func init() {
	registerDataDescriptionProvider("cloud_tke_kubernetes_available_cluster_versions", CNDescription{
		...
		AttributesCN: map[string]string{
			"cluster_id":         "群集Id，cluster_id与cluster_ids必须且只能填写一个",
			"cluster_ids":        "群集ID的列表，cluster_id与cluster_ids必须且只能填写一个",
			"versions":           "可升级的群集版本号注意：此字段可能返回null，表示无法获得有效值",
			"clusters":           "集群信息注意：此字段可能返回null，表示无法获得有效值",
...
func dataSourceTencentCloudKubernetesAvailableClusterVersions() *schema.Resource {
	return &schema.Resource{
		Description: "Use this data source to query detailed information of kubernetes available_cluster_versions",
		Read:        dataSourceTencentCloudKubernetesAvailableClusterVersionsRead,
		Schema: map[string]*schema.Schema{
			"cluster_id": {
				Optional:     true,
				Type:         schema.TypeString,
				ExactlyOneOf: []string{"cluster_ids"},
				Description:  "Cluster Id.  cluster_id and cluster_ids must be set only one.",
			},

			"cluster_ids": {
				Optional:     true,
				Type:         schema.TypeSet,
				ExactlyOneOf: []string{"cluster_id"},
				Elem: &schema.Schema{
					Type: schema.TypeString,
				},
				Description: "List of cluster IDs. cluster_id and cluster_ids must be set only one.",
			},
