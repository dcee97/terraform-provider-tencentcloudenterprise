	...
	defer func() {
		if errRet != nil {
			log.Printf("[CRITAL]%s api[%s] fail, request body [%s], reason[%s]\n",
				logId, request.BaseRequest, request.ToJsonString(), errRet.Error())
		}
	}()
...
func (me *RedisService) UpgradeInstance(ctx context.Context, redisId string, newMemSize, redisShardNum, redisReplicasNum int) (dealId string, errRet error) {
	logId := getLogId(ctx)
...
func (me *RedisService) DescribeRedisInstanceNodeInfoByFilter(ctx context.Context, param map[string]interface{}) (instanceNodeInfo *redis.DescribeInstanceNodeInfoResponse, errRet error) {
	var (
		logId   = getLogId(ctx)
