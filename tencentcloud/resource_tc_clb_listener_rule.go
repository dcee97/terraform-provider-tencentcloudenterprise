...
func init() {
	registerResourceDescriptionProvider("cloud_clb_listener_rule", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"health_check_time_out":      "健康检查超时时间",
			"health_check_http_code":     "健康检查状态码",
...
func resourceTencentCloudClbListenerRule() *schema.Resource {
	return &schema.Resource{
		Description: "Provide a resource to create a CLB listener rule.",
		...
		Schema: map[string]*schema.Schema{
			...
			"forward_type": {
				...
				Description:  "Forwarding protocol between the CLB instance and real server. Valid values: `HTTP`, `HTTPS`.",
			},
			//computed
			"rule_id": {
				Type:        schema.TypeString,
...
func resourceTencentCloudClbListenerRuleCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("scheduler"); ok {
		if !(protocol == CLB_LISTENER_PROTOCOL_HTTP || protocol == CLB_LISTENER_PROTOCOL_HTTPS) {
			return fmt.Errorf("[CHECK][CLB listener rule][Create] check: Scheduler can only be set with listener protocol TCP/UDP or rule of listener HTTP/HTTPS")
		}
	...
	if v, ok := d.GetOk("session_expire_time"); ok {
		...
		if !(vv == 0 || vv >= 30 && vv <= 3600) {
			return fmt.Errorf("[CHECK][CLB listener rule][Create] check: " +
...
func resourceTencentCloudClbListenerRuleUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("scheduler") {
		...
		if !(protocol == CLB_LISTENER_PROTOCOL_HTTP || protocol == CLB_LISTENER_PROTOCOL_HTTPS) {
			return fmt.Errorf("[CHECK][CLB listener rule %s][Update] check: Scheduler can only be set with listener protocol TCP/UDP or rule of listener HTTP/HTTPS", locationId)
		}
