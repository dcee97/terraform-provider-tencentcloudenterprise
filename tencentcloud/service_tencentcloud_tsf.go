...
func (me *TsfService) DescribeTsfClusterById(ctx context.Context, clusterId string) (cluster *tsf.ClusterV2,
	errRet error) {
	logId := getLogId(ctx)
...
func (me *TsfService) DescribeTsfApplicationFileConfigById(ctx context.Context, configId,
	configName string) (applicationFileConfig *tsf.FileConfig, errRet error) {
	logId := getLogId(ctx)
	...
	if configId == "" && configName == "" {
		errRet = fmt.Errorf("configId and configName can not both be empty")
		return
	}
