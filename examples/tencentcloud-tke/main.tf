...
resource "cloud_kubernetes_cluster" "cluster_with_addon" {
  ...
  managed_cluster_internet_security_policies = ["203.0.113.3", "203.0.113.1"]
  cluster_deploy_type                        = "MANAGED_CLUSTER"
...
resource "cloud_kubernetes_cluster" "managed_vpc_cni_cluster" {
...
resource "cloud_kubernetes_cluster" "independing_cluster" {
  vpc_id                  = var.vpc
  cluster_cidr            = "203.0.113.0/24"
  cluster_max_pod_num     = 32
  cluster_name            = "test"
...
resource cloud_kubernetes_scale_worker test_scale {
...
resource "cloud_kubernetes_as_scaling_group" "test" {
